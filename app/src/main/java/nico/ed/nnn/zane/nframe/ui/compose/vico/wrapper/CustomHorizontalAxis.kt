 ...
 * @see Axis
 */
class CustomHorizontalAxis<Position : AxisPosition.Horizontal>(
     ...
     */
    @Suppress("DEPRECATION")
    var itemPlacer: AxisItemPlacer.Horizontal =
    ...
    override fun drawBehindChart(context: ChartDrawContext): Unit = with(context) {
        ...
        canvas.clipRect(
            bounds.left - itemPlacer.getStartHorizontalAxisInset(
                this,
                horizontalDimensions,
                tickThickness
            ),
            minOf(bounds.top, chartBounds.top),
            bounds.right + itemPlacer.getEndHorizontalAxisInset(
                this,
                horizontalDimensions,
                tickThickness
            ),
        ...
        labelValues.forEachIndexed { index, x ->
            ...
            val maxWidth = (min(
                x - previousX,
                nextX - x
            ) / chartValues.xStep * horizontalDimensions.xSpacing).toInt()

            // 最後のラベルの描画に使う TextComponent は恣意的に labels の最後のものに置き換える
            (if (index == labelValues.lastIndex) labels?.last() else labels?.first())?.drawText(
                context = context,
    ...
    private fun getDesiredHeight(
        ...
        when (val constraint = sizeConstraint) {
            ...
            is SizeConstraint.TextWidth -> labels?.first()?.getHeight(
                context = this,
     ...
     */
    class Builder<Position : AxisPosition.Horizontal>(
        builder: CustomAxis.Builder<Position>? = null,
    ) : CustomAxis.Builder<Position>(builder) {
        ...
        var labelSpacing: Int = 1
            set(value) {
                ...
                @Suppress("DEPRECATION")
                itemPlacer =
                    AxisItemPlacer.Horizontal.default(spacing = value, offset = labelOffset)
            }
        ...
        var labelOffset: Int = 0
            set(value) {
                ...
                @Suppress("DEPRECATION")
                itemPlacer =
                    AxisItemPlacer.Horizontal.default(spacing = labelSpacing, offset = value)
            }
        ...
        inline fun <reified T : Position> build(): CustomHorizontalAxis<T> {
            ...
            @Suppress("DEPRECATION")
            return setTo(CustomHorizontalAxis(position = position)).also { axis ->
                axis.labelSpacing = labelSpacing
                axis.labelOffset = labelOffset
                axis.itemPlacer = itemPlacer
            } as CustomHorizontalAxis<T>
        }
    }
 ...
 */
inline fun <reified Position : AxisPosition.Horizontal> createHorizontalAxis(
    block: CustomHorizontalAxis.Builder<Position>.() -> Unit = {},
): CustomHorizontalAxis<Position> = CustomHorizontalAxis.Builder<Position>().apply(block).build()
